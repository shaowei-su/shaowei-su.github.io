...
class Solution {
    public int numFriendRequests(int[] ages) {
        ...
        for (Integer a : ageCount.keySet()) {
            for (Integer b : ageCount.keySet()) {
                count += numReqs(a.intValue(), b.intValue()) * ageCount.get(a) * (ageCount.get(b) - (a == b ? 1 : 0));
            }
        }
        return count;
    }
    public int numReqs(int a, int b) {
        if (b > a) return 0;
        if (b > 100 && a < 100) return 0;
        if (b <= (0.5 * a) + 7) return 0;
        return 1;
    }
}
