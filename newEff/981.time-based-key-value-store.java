...
class TimeMap {
    ...
    public void set(String key, String value, int timestamp) {
       TreeMap<Integer, String> tMap = timeMap.computeIfAbsent(key, m -> new TreeMap<>());
       tMap.put(timestamp, value);
    }
    
    public String get(String key, int timestamp) {
        if (timeMap.containsKey(key)) {
            Integer nearest = timeMap.get(key).floorKey(timestamp);
            if (nearest == null) {
                return "";
            }
            return timeMap.get(key).get(nearest);
        } else {
            return null;
        }
    }
