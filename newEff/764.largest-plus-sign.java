...
class Solution {
    public int orderOfLargestPlusSign(int N, int[][] mines) {
        ...
        Set<String> mineSet = new HashSet<>();
        for (int[] mine : mines) {
    ...
    private boolean isCross(Set<String> mineSet, int row, int col, int len) {
        for (int i = row; i <= row + len; i++) {
            if (mineSet.contains(i + "," + col)) return false;
        }
        for (int i = row; i >= row - len; i--) {
            if (mineSet.contains(i + "," + col)) return false;
        }
        for (int i = col; i <= col + len; i++) {
            if (mineSet.contains(row + "," + i)) return false;
        }
        for (int i = col; i >= col - len; i--) {
            if (mineSet.contains(row + "," + i)) return false;
        }
