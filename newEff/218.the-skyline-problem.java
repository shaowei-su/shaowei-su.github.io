...
class Solution {
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        for (int[] height : heights) {
            ...
            if (prev != curMax) {
                res.add(new int[] {height[0], curMax});
                prev = curMax;
            }
        }
        return res;
    ...
    public List<int[]> getSkyline2(int[][] buildings) {
        PriorityQueue<int[]> minHeap = new PriorityQueue<>((a, b) -> a[0] == b[0] ? (a[2] == b[2] ? a[1] - b[1] : b[2] - a[2]) : a[0] - b[0]);
        for (int[] b : buildings) {
            minHeap.offer(b);
        }
        List<int[]> res = new ArrayList<>();
        ...
        while (minHeap.size() > 0) {
            ...
            if (prev == null) {
                res.add(new int[]{cur[0], cur[2]});
            } else {
                /**
                 * 1. no overlap 
                 * 2. second one is nested with k
                 **/
                if (prev[1] < cur[0]) {
                    res.add(new int[]{prev[1], 0});
                    res.add(new int[]{cur[0], cur[2]});
                } else {
                    if (prev[2] > cur[2]) {
                    } 
