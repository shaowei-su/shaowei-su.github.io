...
class SnakeGame {
    ...
    public int move(String direction) {
        ...
        int[] firstFood = foodList.size() > 0 ? foodList.peekFirst() : new int[] {-1, -1};
        if (next[0] < 0 || next[0] >= height || next[1] < 0 || next[1] >= width) {
            return -1;
        }
        if (occupied.contains(next[0] + "-" + next[1])) {
            return -1;
        }
        occupied.add(next[0] + "-" + next[1]);
        if (firstFood[0] == next[0] && firstFood[1] == next[1]) {
