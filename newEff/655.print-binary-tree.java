...
class Solution {
    ...
    public void helper(TreeNode root, List<List<String>> res, int depth, int i, int j) {
        if (root == null) {
            return;
        }
        int mid = i + (j - i) / 2;
        res.get(depth).set(mid, String.valueOf(root.val));
        helper(root.left, res, depth + 1, i, mid - 1);
        helper(root.right, res, depth + 1, mid + 1, j);
    }
