...
class RandomizedSet {
    ...
    public boolean insert(int val) {
        if (keyPos.containsKey(val)) {
            return false;
        }
        keyPos.put(val, valList.size());
        valList.add(val);
        return true;
    }
    ...
    public boolean remove(int val) {
       ...
       int pos = keyPos.get(val);
       int lastInList = valList.get(valList.size() - 1);
       valList.set(pos, lastInList);
       valList.remove(valList.size() - 1);
       keyPos.put(lastInList, pos);
       keyPos.remove(val);
       return true;
    }
